//! WasmPluginHost — wasmtime load + `jereko_hook` export ABI (WASI-ready linker seam).
...
/// WasmPluginHost — loads wasm modules and invokes the `jereko_hook` export when present.
...
impl WasmPluginHost {
    ...
    fn invoke_export(bytes: &[u8], hook: &HookCall) -> PluginResult<serde_json::Value> {
        ...
        .map_err(|e| PluginError::Host {
            host: "wasm".into(),
            message: format!("jereko_hook call: {e}"),
        })?;
...
(module
  ...
  (func (export "jereko_hook") (param i32 i32) (result i32)
    i32.const 16)
  (data (i32.const 16) "{\"host\":\"wasm\",\"hook\":\"before_transform\",\"stub\":false,\"abi\":\"jereko_hook\"}\00")
)
"#;
    ...
    #[tokio::test]
    async fn invokes_jereko_hook_export() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let result = host
            ...
            .unwrap();
        assert_eq!(result.output["stub"], false);
        assert_eq!(result.output["abi"], "jereko_hook");
