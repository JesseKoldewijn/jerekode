...
use jerekode_core::{Message, MessageRole};
use jerekode_plugins::{
    HookCall, PluginOrchestrator, TOOL_EXECUTE_BEFORE, apply_command_mutations, set_command_arg,
};
use jerekode_providers::{CompletionChunk, CompletionRequest, ProviderRegistry, resolve};
use std::sync::Arc;
...
pub struct StreamMessageResult {
    ...
    pub session: jerekode_core::Session,
...
impl HandlerContext {
    ...
    pub fn get_session(
        ...
        let id = jerekode_core::SessionId(session_id.to_string());
        let session = self
            .sessions
            .get(&id)
    ...
    pub fn list_messages(
        &self,
        session_id: &str,
    ) -> Result<Vec<jerekode_core::Message>, HandlerError> {
