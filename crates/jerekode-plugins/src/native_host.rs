...
impl PluginHost for NativePluginHost {
    ...
    async fn load(&self, spec: &PluginSpec) -> PluginResult<LoadedPlugin> {
        ...
        if std::path::Path::new(&path).exists() {
            // SAFETY: plugin authors must export the C ABI from jerekode_plugin.h.
    ...
    async fn invoke_hook(&self, plugin: &LoadedPlugin, hook: HookCall) -> PluginResult<HookResult> {
        ...
        if let Some(lib) = libraries.get(&plugin.spec.name) {
            type InfoFn = unsafe extern "C" fn() -> JerekodePluginInfo;
            type InvokeFn = unsafe extern "C" fn(
                *const std::ffi::c_char,
                *const std::ffi::c_char,
            ) -> JerekodeHookResult;

            // SAFETY: symbols match jerekode_plugin.h.
            let invoke: libloading::Symbol<InvokeFn> = unsafe {
                lib.get(b"jerekode_plugin_invoke\0").map_err(|e| {
                    PluginError::Native(format!("missing jerekode_plugin_invoke: {e}"))
                })?
            };
            let _info: libloading::Symbol<InfoFn> = unsafe {
                lib.get(b"jerekode_plugin_info\0").map_err(|e| {
                    PluginError::Native(format!("missing jerekode_plugin_info: {e}"))
                })?
            };
...
mod tests {
    ...
    async fn loads_invokes_and_unloads_test_dylib() {
        ...
        if !path.exists() {
            let msg = format!(
                "native dylib missing at {}; build jerekode-test-native-plugin first",
                path.display()
            );
