...
1. **OpenCode2 / Bun** TypeScript plugin (`tool.execute.before`) — usable by upstream OpenCode2 and Jereko’s Bun sidecar  
2. **Native** cdylib (`jereko-rtk-plugin`) — `NativePluginHost` via `jereko-plugin-sdk`
...
Jereko’s HTTP `/tools/execute` path dispatches `tool.execute.before` through `PluginOrchestrator` before bash runs ([ADR 002](./002-dual-plugin-runtime.md)).
